...
public class BevShopNoGUITest {
	...
	public static void main(String[] args) {
		...
		b.startNewOrder(12, DAY.day.MONDAY, "John", 23);
		...
		if (b.validAge(tempAge)) {
			...
			b.processAlcoholOrder("Mohito", SIZE.size.SMALL);
		}
		...
		System.out.println("Add second alcohol drink"); // Add second alcohol
														// drink
		b.processAlcoholOrder("tonic", SIZE.size.LARGE);
		...
		System.out.println("Add third alcohol drink"); // Add third alcohol
														// drink
		b.processAlcoholOrder("wine", SIZE.size.SMALL);
		...
		b.processCoffeeOrder("cappuchino", SIZE.size.SMALL, true, true);
		System.out.println(b.getNumOfAlcoholDrink()); // 3

		System.out.println("Total on the Order:" + b.getCurrentOrder().calcOrderTotal()); // 11.0
		tempOrderNo = b.getCurrentOrder().getOrderNum();
		...
		if (b.validAge(b.getCurrentOrder().getCustomer().getAge())) {
			...
			b.processAlcoholOrder("mohito", SIZE.size.MEDIUM);
		} else
		...
		b.processSmoothieOrder("Detox", SIZE.size.LARGE, numOfFruits, false);
