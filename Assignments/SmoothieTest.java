...
public class SmoothieTest {
	...
	public void setUp() throws Exception {
		s1 = new  Smoothie ("s1Name", SIZE.size.SMALL, 1,false );
		s2 = new  Smoothie ("s2NAme", SIZE.size.MEDIUM, 2,true );
		s3 = new  Smoothie ("s3NAme", SIZE.size.LARGE, 3, true );
		s4 = new  Smoothie ("s4NAme", SIZE.size.SMALL, 4, false );
	...
	public void testToString() {
		...
		assertTrue(s1.toString().contains(s1.getName()));
		assertTrue(s1.toString().contains(String.valueOf(s1.calcPrice()) ));
		assertTrue(s1.toString().contains(s1.getBeverageSize().toString() ));
